...
A fresh clone downloads a versioned public archive and checksum-validates the
five raw datasets used by the numerical analyses. Evidence-only snapshots that
cannot be redistributed are governed by `SOURCE_ARCHIVE_POLICY.md`. Authors
with the preserved local archive can run `make audit-full-source-archive` to
require all registered snapshots.
