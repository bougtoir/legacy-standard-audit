...
`make all` retrieves a versioned public release archive and SHA-256 verifies
the archive and every raw input used by the numerical analyses:
...
These five datasets plus their release archive are the
reproducibility-critical source set. A missing record, download failure, size
mismatch, or checksum mismatch stops the build. The ISO 216 result is generated
from the stated repeated-halving equation; its copyrighted sample is
evidence-only and is not required to execute that mathematical null control.
